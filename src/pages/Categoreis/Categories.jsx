...
const Categories = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Action:", action); // Log the action to see what it returns
      if (createCategory.fulfilled.match(action)) {
        setNewCategory({ name: '', logo: null }); // Reset state after successful creation
        toast.success('Category added successfully');
        await dispatch(fetchCategories({})); // Refresh the categories
      } else {
        toast.error(action.payload?.message || 'Failed to add category');
      }
    } catch (error) {
      toast.error('Failed to add category');
