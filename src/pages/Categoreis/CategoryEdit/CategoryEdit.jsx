...
//         if (category.logo) {
//           viewer.src = `http://localhost:5000/uploads/${category.logo}`;
//         } else {
    ...
    axios.get(`${ApiUrl}categories/${id}`)
      .then(response => {
        ...
        setCategoryData({
          name: category.name || '',
          priority: category.priority || 0,
          logo: null, // Don't store logo directly, only manage it via file upload
        });
        // Set the image preview if available
        const viewer = document.getElementById('viewer');
        if (category.logo) {
          viewer.src = `http://localhost:3000/uploads/${category.logo}`;
        } else {
          viewer.src = '/image-place-holder.png';
        }
      })
